...
/**
 * Filters by given list of news muncipality options.
 *
 * @ingroup views_filter_handlers
 *
 * @ViewsFilter("d8views_news_municipality")
 */
class NewsMunicipality extends InOperator {
  ...
  public function generateOptions() {
    ...
    if (isset($exposed_input['news_keywords'])) {
      if ($exposed_input['news_keywords'] != 'All') {
        $query->condition('field_keywords.entity.tid', $exposed_input['news_keywords']);
      }
    }
    ...
    if ($nids) {
      ...
      foreach ($nodes as $node) {
        if ($node->get('field_municipalities')->referencedEntities()) {
          foreach ($node->get('field_municipalities')->referencedEntities() as $item) {
            if (!empty($item)) {
              ...
              $municipalities[$name_translated] = $option;
            }
          }
        }
      }
