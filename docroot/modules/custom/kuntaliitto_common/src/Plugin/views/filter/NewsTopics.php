...
/**
 * Filters by given list of news topics options.
 *
 * @ingroup views_filter_handlers
 *
 * @ViewsFilter("d8views_news_topics")
 */
class NewsTopics extends InOperator {
  ...
  public function generateOptions() {
    ...
    if ($nids) {
      ...
      foreach ($nodes as $node) {
        if ($node->get('field_topics')->referencedEntities()) {
          foreach ($node->get('field_topics')->referencedEntities() as $item) {
            if (!empty($item)) {
